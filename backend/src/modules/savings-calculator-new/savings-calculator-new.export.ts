  ...
  static async exportToExcel(id: string, monthStr?: string, version?: number): Promise<Buffer> {
    ...
    if (monthStr === 'all') {
      ...
      if (allResults.length > 0) {
        ...
        await SavingsCalculatorNewExportService.populateSummarySheet(summarySheet, entry, allResults, monthRowMap);
      }
    } else {
      const entry = await SavingsCalculatorNewService.getEntryOrVersion(id, version);
      ...
      await SavingsCalculatorNewExportService.addSavingsSheet(workbook, sheetName, result, entry, monthStr);
    }

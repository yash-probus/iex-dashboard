...
function findCustomSlotForTime(slots: any[], hour: number, minute: number) {
...
export class CustomSavingCalcService {
  static async getAll() {
  ...
  static async update(id: string, data: {
    ...
    return prisma.$transaction(async (tx) => {
      ...
      await invalidateCache(`market-custom:${id}:*`);
      await invalidateCache(`demandshift-custom:${id}:*`);
      await invalidateCache(`calc-custom:savings:${id}:*`);
  ...
  static async getClientOverview(id: string) {
    ...
    for (const month of months) {
      try {
        const result = await CustomSavingCalcService.calculateMarketDecision(id, month);
  ...
  static async calculateSavingsHV2(entry: any, targetMonth?: string, shiftInsights?: any) {
    ...
    for (const [yearMonth, monthConsumptions] of monthsToProcess) {
      ...
      let startDayInput = monthConsumptions.startDate || monthConsumptions['Start Date'];
      let endDayInput = monthConsumptions.endDate || monthConsumptions['End Date'];
      const slots = (monthConsumptions as any).slots || [];
      ...
      const slotsData = records.map(rec => {
        ...
        const matchedSlot = findCustomSlotForTime(slots as any[], hour, minute);
        if (matchedSlot) {
          discomLandingPrice = Number(matchedSlot.effectivePrice || 7.5);
          matchedTariffName = matchedSlot.name || 'NORMAL';
        } else {
          discomLandingPrice = 7.5;
          matchedTariffName = 'NORMAL';
        }
      ...
      Object.keys(slotsByTod).forEach(groupKey => {
        ...
        if (shiftInsights) {
          ...
          const matchedSlot = slots.find((s: any) => s.name.toUpperCase() === groupKey.toUpperCase());
          if (matchedSlot) {
            remainingEnergy = Number(matchedSlot.consumption || 0);
          } else {
  ...
  static async calculateMarketDecision(id: string, targetMonthStr?: string, version?: number, useShiftedProfile: boolean = false, shiftInsights?: any) {
    ...
    const cacheKey = `market-custom:${id}:v:${cacheVersion}:m:${targetMonthStr || 'default'}`;
    ...
    const slots = monthConsumptions.slots || [];
    let tariffs: any[] = [];
    ...
    const slotsData = records.map(rec => {
      ...
      const matchedSlot = findCustomSlotForTime(slots as any[], hour, minute);
      if (matchedSlot) {
        discomBase = Number(matchedSlot.effectivePrice || 7.5);
        matchedTariffName = matchedSlot.name || 'NORMAL';
      } else {
        discomBase = 7.5;
        matchedTariffName = 'NORMAL';
  ...
  static async calculateDemandShiftInsights(id: string, targetMonth?: string, version?: number) {
    ...
    const cacheVersion = version !== undefined ? version : 'live';
    const cacheKey = `demandshift-custom:${id}:v:${cacheVersion}:m:${targetMonth || 'default'}`;
    const cached = await getCache(cacheKey);
    if (cached && false) {
      return cached;
    }

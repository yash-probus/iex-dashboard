...
export class TraderPerformanceActualExportService {
  ...
  static async exportToExcel(id: string, monthStr?: string, version?: number, isActualTrader = false): Promise<Buffer> {
    ...
    if (monthStr === 'all') {
      ...
      const months = Object.keys(entry?.todConsumptions || {}).filter(k => /^\d{4}-\d{2}$/.test(k)).sort();
      const allResults = [];
      
      for (const m of months) {
        const result = await TraderPerformanceActualService.calculateMarketDecision(id, m, version);
      ...
      if (allResults.length > 0) {
        ...
        for (const r of allResults) {
          ...
          const rowMapping = await TraderPerformanceActualExportService.addSavingsSheet(workbook, sheetName, r.result, entry, r.monthStr, isActualTrader);
          monthRowMap[r.monthStr] = { sheetName, ...rowMapping };
        }
        
        await TraderPerformanceActualExportService.populateSummarySheet(summarySheet, entry, allResults, monthRowMap, isActualTrader);
      }
    } else {
      const entry = await TraderPerformanceActualService.getEntryOrVersion(id, version);
      const result = await TraderPerformanceActualService.calculateMarketDecision(id, monthStr, version);
      const sheetName = getShortSheetName(monthStr, 'Savings Analysis');
      await TraderPerformanceActualExportService.addSavingsSheet(workbook, sheetName, result, entry, monthStr, isActualTrader);
    }
  ...
  private static async populateSummarySheet(sheet: ExcelJS.Worksheet, entry: any, allResults: any[], monthRowMap: Record<string, any>, isActualTrader = false) {
    ...
    todSlabs.forEach(tod => {
      ...
      allResults.forEach((r, idx) => {
        ...
        const formula = `IFERROR(VLOOKUP("${tod}", '${mMapping.sheetName}'!$A$${mMapping.breakdownTableStart}:$L$${mMapping.breakdownTableEnd}, 8, FALSE), 0)`;
  ...
  static async exportDemandShiftToExcel(id: string, monthStr?: string, version?: number): Promise<Buffer> {
    ...
    if (monthStr === 'all') {
      ...
      for (const m of months) {
        const result = await TraderPerformanceActualService.calculateMarketDecision(id, m, version, true);
        allResults.push({ monthStr: m, result });
      }
      ...
      if (allResults.length > 0) {
        ...
        for (const r of allResults) {
          ...
          const rowMapping = await TraderPerformanceActualExportService.addSavingsSheet(workbook, sheetName, r.result, entry, r.monthStr);
          monthRowMap[r.monthStr] = { sheetName, ...rowMapping };
        }
        
        await TraderPerformanceActualExportService.populateSummarySheet(summarySheet, entry, allResults, monthRowMap);
      }
    } else {
      const entry = await TraderPerformanceActualService.getEntryOrVersion(id, version);
      const result = await TraderPerformanceActualService.calculateMarketDecision(id, monthStr, version, true);
      const sheetName = getShortSheetName(monthStr, 'Demand Shift');
      await TraderPerformanceActualExportService.addSavingsSheet(workbook, sheetName, result, entry, monthStr);
    }

...
for file in files:
    ...
    # Find where the loops start and inject isKvahBilling
    ...
    slotsData.forEach"""
        content = content.replace("slotsData.forEach", injection, 1)

    # Patch slabConsumption assignments
    # We want to find cases where slabConsumption is assigned from flatTotal or matchedKey, and multiply by pf if isKvahBilling
    
    # 1. slabConsumption = flatTotal * (slotsByTod[groupKey].length / totalSlotsInMonth);
    ...
    # 2. slabConsumption = flatTotal * (slotsInGroup.length / totalSlotsInMonth);
    ...
    # 3. slabConsumption = Number(monthConsumptions[matchedKey]);

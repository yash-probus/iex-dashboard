...
async function trySearch(url: string) {
  ...
  try {
    ...
    if (ssnEntries.length > 0) {
      console.log("Found matching entry in OLD!", ssnEntries);
    }
    ...
    console.log(newEntries.map(e => ({ id: e.id, clientName: e.clientName, industryName: (e as any).industryName })));
    
    const ssnNewEntries = newEntries.filter(e => 
      e.clientName.toLowerCase().includes('ssn') || 
      e.clientName.toLowerCase().includes('mayank') || 
      ((e as any).industryName && (e as any).industryName.toLowerCase().includes('ssn'))
    );
    if (ssnNewEntries.length > 0) {
      console.log("Found matching entry in NEW!", ssnNewEntries);
    }
...
async function run() {
  const urls = [
    'postgresql://postgres:postgres@localhost:5432/iex_dashboard',
    'postgresql://postgres:iex_sec_k9P2mX_2026@localhost:5432/iex_dashboard'
  ];

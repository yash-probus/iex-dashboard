...
(async () => {
  ...
  if (selectHandles.length >= 2) {
    ...
    await page.evaluate(() => {
       ...
       if (inputs.length >= 2) {
         ...
         if (nativeInputValueSetter) {
             nativeInputValueSetter.call(inputs[0], '01-07-2026');
             inputs[0].dispatchEvent(new Event('input', { bubbles: true }));
             nativeInputValueSetter.call(inputs[1], '01-07-2026');
    ...
    const pagination = await page.evaluate(() => {
       return Array.from(document.querySelectorAll('button, p')).map(e => e.textContent).filter(t => t && t.includes('of'));
    });
    console.log('Pagination info:', pagination);
    
    const rows = await page.evaluate(() => document.querySelectorAll('table tbody tr').length);
    console.log('Rows found for 01-07-2026:', rows);

...
import {
  fetchCustomSavingCalcEntries,
  createCustomSavingCalcEntry,
  updateCustomSavingCalcEntry,
  deleteCustomSavingCalcEntry,
  fetchCustomSavingCalcEntryById,
  fetchEntryHistoryCustom,
  fetchResourceDefaults,
  CustomSavingCalcEntry,
  CustomTodSlot
...
export default function CustomSavingCalcPage() {
  const navigate = useNavigate();

  const [entries, setEntries] = useState<CustomSavingCalcEntry[]>([]);
  ...
  const loadEntries = async () => {
    ...
    setError(null);
    try {
      const data = await fetchCustomSavingCalcEntries();
      setEntries(data || []);
    } catch (err: any) {
  ...
  const handleOpenEdit = (entry: CustomSavingCalcEntry) => {
    setDialogMode('edit');
  ...
  const handleSaveEntry = async () => {
    ...
    try {
      ...
      if (dialogMode === 'create') {
        await createCustomSavingCalcEntry(payload);
      } else if (selectedEntryId) {
        await updateCustomSavingCalcEntry(selectedEntryId, payload);
      }
  ...
  const handleOpenHistory = async (entry: CustomSavingCalcEntry) => {
    setHistoryClientName(entry.clientName);
    setHistoryDialogOpen(true);
    setHistoryLoading(true);
    try {
      const data = await fetchEntryHistoryCustom(entry.id);
      setHistoryData(data);
    } catch (error) {
      console.error(error);
  ...
  const handleDeleteEntry = async (id: string) => {
    if (window.confirm('Are you sure you want to delete this entry?')) {
      try {
        await deleteCustomSavingCalcEntry(id);
        loadEntries();
      } catch (err: any) {
  ...
  const columns: ColumnDefinition[] = [
    {
      ...
      width: 90,
      sticky: true,
      renderCell: (row: CustomSavingCalcEntry) => {
      ...
      renderCell: (row: CustomSavingCalcEntry) => (
        <Box sx={{ display: 'flex', gap: 0.5, justifyContent: 'center', alignItems: 'center' }}>
          <Button
            ...
            onClick={() => navigate(`/custom-saving-calc/${row.id}/analysis`)}
            sx={{
              fontSize: '11px',
              py: 0.5,
              borderColor: '#8B5CF6',

...
export default function CustomSavingCalcAnalysisPage() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();

  const [calcEntry, setCalcEntry] = useState<CustomSavingCalcEntry | null>(null);
  ...
  const loadData = async () => {
    ...
    setCalculating(true);
    try {
      const baseEntryData = await fetchCustomSavingCalcEntryById(id);
      
      let versions: number[] = calcVersions;
      if (calcVersions.length === 0) {
        try {
          const history = await fetchEntryHistoryCustom(id);
    ...
    <Box sx={{ p: 3, width: '100%', maxWidth: '383mm', mx: 'auto', '@media print': { maxWidth: '383mm', p: 0 } }}>
      <Box className="dashboard-content-to-hide">
        ...
        <Box className="no-print" sx={{ display: 'flex', alignItems: 'center', mb: 3, gap: 2 }}>
          <IconButton
            onClick={() => navigate('/custom-saving-calc')}
            ...
            <Box className="no-print" sx={{ 
              ...
              <Button
                ...
                onClick={async () => {
                  if (!id) return;
                  try {
                    await exportSavingsExcelCustom(id, selectedSimMonth || undefined, selectedCalcVersion || undefined, calcEntry?.clientName);
                  } catch (err: any) {
              ...
              <Button
                ...
                onClick={async () => {
                  if (!id) return;
                  try {
                    await exportDemandShiftExcelCustom(id, selectedSimMonth || undefined, selectedCalcVersion || undefined, calcEntry?.clientName);
                  } catch (err: any) {
                    setSnackbar({ open: true, message: err.message || 'Excel export failed', severity: 'error' });
                  }
                }}

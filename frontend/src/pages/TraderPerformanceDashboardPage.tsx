...
import {
  ...
  calculateMarketDecisionTraderPerformance,
...
export default function TraderPerformanceDashboardPage() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();

  const [calcEntry, setCalcEntry] = useState<TraderPerformanceEntry | null>(null);
  ...
  const loadData = async () => {
    ...
    setCalculating(true);
    try {
      const baseEntryData = await fetchTraderPerformanceEntryById(id);
      
      let versions: number[] = calcVersions;
      if (calcVersions.length === 0) {
        try {
          const history = await fetchEntryHistoryTraderPerformance(id);
      ...
      const entryData = await fetchTraderPerformanceEntryById(id, selectedCalcVersion || undefined);
      setCalcEntry(entryData);

      // Fetch overview independently, saving massive memory overhead
      const overview = await fetchClientOverviewTraderPerformance(id);
            ...
            <Box className="no-print" sx={{ 
              ...
              <Button
                ...
                onClick={async () => {
                  if (!id) return;
                  try {
                    await exportTraderPerformanceExcel(id, selectedSimMonth || undefined, selectedCalcVersion || undefined, calcEntry?.clientName);
                  } catch (err: any) {
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    for (const m of monthsToFetch) {
                      if (!newCache[m]) {
                        const marketRes = await calculateMarketDecisionTraderPerformance(id, m, selectedCalcVersion || undefined);
          ...
          {marketDecisionResult && !calculating && (
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3.5 }}>
              <Box id="dashboard-screenshot-target" sx={{ mt: 1, bgcolor: '#F8FAFC', p: 2, borderRadius: 2 }}>
                {/* Custom Trader Performance 3-Way Comparison */}
                {calcEntry && marketDecisionResult && (
                  <Box sx={{ mb: 4 }}>
                          ...
                          <Table size="small">
                            <TableHead sx={{ bgcolor: '#F8FAFC' }}>
                              <TableRow>
                                ...
                                <TableCell align="right"><strong>Discom (Original)</strong></TableCell>
                                <TableCell align="right"><strong>Probus (Discom + OA)</strong></TableCell>
                                <TableCell align="right"><strong>Actual Trader (PDF)</strong></TableCell>
                              </TableRow>
                            </TableHead>
                            <TableBody>
                              {(() => {
                                ...
                                return (
                                  <>
                                    ...
                                    <TableRow>
                                      <TableCell><strong>Net Savings vs Discom (₹)</strong></TableCell>

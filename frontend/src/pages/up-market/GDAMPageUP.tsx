...
import { useUpMarketData } from '../../hooks/useUpMarketData';
import { exportToCSV } from '../../utils/export';
import { formatOverviewDate, formatTimeblock } from '../../utils/date';
import { useAuth } from '../../contexts/AuthContext';

const GDAM_ACCENT = '#10B981'; // Green for GDAM
...
export default function GDAMPage() {
  ...
  const { data, summaryMetrics, isLoading, error } = useUpMarketData('GDAM', filters);
    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2.5 }}>
          ...
          <Box>
            ...
            <Typography variant="body1" sx={{ color: 'text.secondary' }}>
              Analyze GDAM market trends, volumes, and prices.
            </Typography>
          </Box>
        </Box>
      ...
      {marketView === 'state-wise' ? (
        <EmptyState 
          ...
          description="State-wise Green Day Ahead Market (GDAM) data is not yet available in the system. We are working on integrating state-level market clearing volumes, purchase/sell bids, and prices."
        />
      ) : error ? (
        <EmptyState 
